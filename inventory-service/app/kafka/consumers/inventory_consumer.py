...
from app.models.inventory_model import Inventory, InventoryUpdate, InventoryStatus
from app.crud.inventory_crud import add_inventory, update_inventory, delete_inventory_by_id, get_inventory_by_id, get_inventory_by_product_id
...
async def process_message(protobuf_product_item: product_pb2.Product, operation: str):
    logger.info(f"Processing message for product_id: {protobuf_product_item.id} with operation: {operation}")
    try:
        with next(get_session()) as session:

            inventory_item = get_inventory_by_product_id(protobuf_product_item.id, session)
            ...
            if operation == "create":
                if inventory_item:
                    ...
                    db_insert_inventory_item = add_inventory(new_inventory_item, session=session)
                    logger.info(f"DB Inserted Inventory Item: {db_insert_inventory_item}")

            elif operation == "update":
                logger.info(f"Updating Inventory Item for product_id {protobuf_product_item.id}")
                if inventory_item:
                    ...
                    db_update_inventory = update_inventory(
                        inventory_item.id, update_data, session=session
                    )
                    logger.info(f"DB Updated Inventory Item: {db_update_inventory}")
                else:
                    logger.error(f"Inventory item not found for product_id {protobuf_product_item.id}")

            elif operation == "delete":
                if inventory_item:
                    db_delete_inventory_item = delete_inventory_by_id(inventory_item.id, session=session)
...
async def consume_product(topic, bootstrap_servers, group_id):
    retries = 0
